...
import { RemoteLoadTaskStatusList } from './remote-load-task-status-list';
...
const makeRemoteTaskStatusListModel = (
  length = 2,
): LoadTaskStatusList.Model[] => {
  const list: LoadTaskStatusList.Model[] = [];
...
type SutTypes = {
  sut: RemoteLoadTaskStatusList;
  httpClientSpy: HttpClientSpy;
};

const makeSut = (url = faker.internet.url()): SutTypes => {
  const httpClientSpy = new HttpClientSpy();
  const sut = new RemoteLoadTaskStatusList(url, httpClientSpy);
  return {
    sut,
    httpClientSpy,
  };
...
describe('RemoteLoadTaskList', () => {
  ...
  it('should return a list of TaskModels if HttpClient returns 200', async () => {
    ...
    const statusList = makeRemoteTaskStatusListModel(length);
    httpClientSpy.response = {
